...
Public Class clsController
    ...
    Private aMenu As New SortedList(Of String, clsMenu)
    ' Private Const connection As String = "Data source = (local); Initial Catalog =javProgram3;" &
    ' "Integrated Security=True"
    Private strError As String
    Public accessDataBase As New clsDataBase
    ...
    Public ReadOnly Property getPrices() As String()
        'get menu item price
        Get
            Dim arrMenuPrices(aMenu.Keys.Count - 1) As String
            Dim i As Integer = 0
            For Each key As String In aMenu.Keys
                arrMenuPrices(i) = aMenu.Item(key).getPrice
                i = i + 1
            Next
    ...
    Public ReadOnly Property getMenu() As String()
        ...
        Get
            Dim arrMenuItems(aMenu.Keys.Count - 1) As String
            Dim i As Integer = 0
            For Each key As String In aMenu.Keys
                arrMenuItems(i) = aMenu.Item(key).getDescription
                i = i + 1
            Next
            Return arrMenuItems
        End Get
    ...
    Public Function loadMenu() As String
        ...
        'connect to database
        clsDataBase.getAConnection()
        'retrieve all orders from database
        '        lstDBMenu = clsDataBase.getMenuList()

        For i As Integer = 0 To lstDBMenu.Count - 1
            Dim arrProduct(2) As String
            arrProduct = lstDBMenu(i)

            Dim menu As New clsMenu
            menu.SKU = arrProduct(0)
            menu.description = arrProduct(1)
            menu.price = arrProduct(2)

            aMenu.Add(menu.SKU, menu)
        Next
